...
(`chz.py search-stock`, см. `core`-функцию `get_chz_stock_via_search`):
- тянем коды по нашему ИНН за окно `introducedDatePeriod` = последние
  `SEARCH_STOCK_WINDOW_DAYS` (60) дней (любой статус, в основном RETIRED);
- привязка к бару — по `modId` кода → `ownerMod.kpp` через `/cises/info`
  (НЕ по `kpp`/`applicationMod` — это поставщик); modId резолвится лениво (~4 МОД);
- **early-stop по сходимости**: выведенных кодов десятки тысяч на GTIN, но уникальных
  партий ~40 — после `SEARCH_CONVERGE_PAGES` (25) страниц без новой партии
  останавливаемся (пул кодов 100k+ → ~2-4k, секунды вместо минут);
- результат — тот же формат с `by_kpp`, поэтому остальная логика не меняется.
