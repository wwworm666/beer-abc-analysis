...
- Инкассируем всё сверх минимума на размен `CASH_CHANGE_FLOAT_RUB = 5000 ₽` — в
  каждом баре остаётся размен. Инкассируем только **кратно `INCASS_STEP_RUB =
  1000 ₽`** (сотни/копейки в банкомат не носим — остаток лежит в баре сверх
  размена). Формула: `к инкассации = floor(max(0, наличные − 5000) / 1000) × 1000`.
