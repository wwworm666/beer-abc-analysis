...
### 2026-06-17 — Устойчивость к неполной выдаче `/corporation/groups`

**Problem:** бот слал «ЗАКРЫТ — Варш» при открытой смене Варшавской. Причина —
`/corporation/groups` периодически отдаёт неполный список точек (узлы iiko
рассинхронизированы после смены pos_id у кассы №50, `f4099015`), и открытая
смена с «неизвестным» pos_id молча отбрасывалась (`unknown_pos` при этом пуст).
Подробный разбор — [docs/lessons.md](lessons.md).

**Что:**
- В `check_bars_state` добавлен резерв `pointOfSaleId -> venue_key`, который
  только растёт: базовый `_SEED_POS_MAP` (текущие pos_id 4 баров) + выученные
  привязки на диске `open_check_pos_map.json` (каждая полная выдача дополняет).
- Если у открытой смены `pos_id` нет в живой выдаче — берём из резерва (с
  `log.warning`), а не выкидываем. Молчаливый `continue` заменён на лог.

**Почему:** одна неполная выдача iiko не должна превращаться в ложную тревогу.
Бар переучивается на новый pos_id с первой же полной выдачи.
