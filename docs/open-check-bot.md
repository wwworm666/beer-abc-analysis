...
|---|---|---|
| `iiko_error=True` | Только админам (`TELEGRAM_ALARM_CHAT_IDS`) | `[Open-check ERROR] 14:59 МСК. iiko недоступен: <err>. Проверка не выполнена.` |
| `closed_keys == []` | Только в группу (`TELEGRAM_GROUP_CHAT_ID`) | `[Open-check] 14:59 МСК. Все 4 бара открыты: <названия>.` |
| `closed_keys != []` | Только админам | `[Open-check ALARM] 14:59 МСК. Закрыты: <list>. Открыты: <list>.` |
