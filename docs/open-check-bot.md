...
| Файл | Что делает |
|---|---|
| [core/open_check_bot.py](../core/open_check_bot.py) | Логика проверки, форматирование, отправка с ретраями (sync, через `requests`) |
...
| [core/open_check_pending.py](../core/open_check_pending.py) | Очередь недоставленных отчётов (`open_check_pending.json`): досылка до конца дня с пометкой о задержке |
...
| [core/open_check_scheduler.py](../core/open_check_scheduler.py) | Daemon-threads: ежедневный запуск в 14:59 МСК + catch-up после рестарта + resend-цикл досылки |
...
### 4. Гарантия доставки (после пропуска тревоги 2026-08-16)

Инцидент: 16.08.2026 в 14:59 тревога «ЗАКРЫТ — ВО» дошла только до 2 из 4
получателей — блокировка ТСПУ «мигала» (тот же запасной IP, что дважды дал
таймаут, через секунды заработал у следующего получателя). Отправка была «одна
попытка на чат», провал не логировался с chat_id. Разбор — [lessons.md](lessons.md).

Пять слоёв защиты (в порядке срабатывания):

1. **Ретраи в момент отправки** — `_send_with_retries` в open_check_bot.py:
   `SEND_PASSES = 2` прохода по недоставленным чатам с паузой
   `SEND_RETRY_PAUSE_SEC = 10`. Ровно 2, а не больше: длинный хвост забирает
   очередь досылки. Худший случай по времени (Telegram «висит» на чтении) —
   ~5 минут; ручной `run-now` это переживает только потому, что gunicorn идёт
   с `--worker-class gthread` (его `--timeout 180` — liveness-проверка,
   in-flight запросы не убиваются). Каждый провал логируется со своим
   `chat_id` (`не доставлено chat_id=...`).
2. **Очередь досылки** — [core/open_check_pending.py](../core/open_check_pending.py):
   недоставленные чаты сохраняются в `open_check_pending.json` (постоянный диск,
   переживает рестарт), resend-поток планировщика раз в `RESEND_INTERVAL_SEC =
   300` досылает остаток, пока не доставит всем или не кончится день. Досланное
   сообщение получает честную пометку:
   `Доставлено с задержкой: отправка в 14:59 МСК не прошла (Telegram был недоступен)`.
   В новый день остаток удаляется с ERROR-логом (текст «Проверка 14:59 МСК»
   завтра стал бы враньём). Лок НЕ держится на время сетевых отправок (иначе
   при лежащем Telegram он висел бы минутами и `add()` со свежей тревогой его
   не дождался бы): под коротким локом ставится **клейм** (`claimed_at`),
   отправка идёт без лока, результат сливается обратно под локом. Чужой свежий
   клейм → `busy`; устаревший (`_CLAIM_TTL_SEC = 600`, процесс умер во время
   досылки) переигрывается — возможен дубль, дубль лучше потери. Процессная
   память доставок (`_delivered_cache`) страхует от дублей, если файл очереди
   не переписался (диск полон). DRY-RUN и ручной `run-now`
   (`queue_failures=False`) в очередь не пишут: тестовый прогон в 10:00 при
   лежащем Telegram иначе поставил бы в очередь тревогу «ЗАКРЫТЫ все» (бары
   ещё не открылись), и после восстановления связи она разошлась бы всем.
3. **Catch-up после рестарта** — `_catch_up_if_missed` в scheduler: если процесс
   стартовал ПОСЛЕ 14:59, а lock за сегодня не взят (рестарт/деплой поверх
   времени проверки) — проверка запускается сразу, в сообщение дописывается
   `Проверка должна была пройти в 14:59 МСК, выполнена позже из-за перезапуска
   сервиса`. Раньше такой день молча терялся (планировщик ждал завтрашних 14:59).
4. **Recovery прерванного прогона** — `_recover_interrupted_run` в scheduler:
   дневной lock живёт по циклу `running` (при взятии) → `done` (прогон
   завершён, включая обработанное падение). Если воркер убит ПОСЛЕ взятия
   lock-а, но ДО завершения (SIGTERM деплоя, рецикл gunicorn
   `--max-requests 800`) — crash-тревога сработать не может (процесс мёртв), а
   catch-up заблокирован существующим lock-ом. Resend-цикл раз в 5 минут
   проверяет: lock `running` старше `_RUN_GRACE_SEC = 900` (дольше любого
   реального прогона) → проверка переигрывается с пометкой
   `была прервана перезапуском сервиса — выполнена повторно`. Lock-и старого
   формата (только pid) считаются завершёнными — деплой этой версии поверх уже
   отработавшего дня дубля не даёт. Принятый остаток: смерть между доставкой и
   записью `done` даёт один дубль (с пометкой) — дубль лучше потери.
5. **Тревога при падении самой проверки** — `send_crash_alarm`: исключение из
   `run_check` (и `OSError` при создании lock-файла — диск полон) шлёт
   получателям тревог `!!! ОШИБКА: проверка баров упала !!!` (с теми же
   ретраями и очередью), а не только print в лог. В DRY-RUN — с префиксом
   `[DRY-RUN]` и одному получателю, как у send_report.

Ошибки записи очереди (диск полон, ФС read-only) не роняют отправку: `add()` —
best-effort, полный текст недоставленного сообщения при этом остаётся в
ERROR-логе для ручного восстановления.

Остаточный риск (принято, без второго канала): VPS лежит весь день, Telegram
недоступен до конца дня или забанен токен — сообщение не придёт вовсе.
Тесты всей цепочки (36) — [tests/test_open_check_delivery.py](../tests/test_open_check_delivery.py).
Дизайн слоёв 4-5 и клейма — результат адверсариального ревью изменений
(13 подтверждённых находок, все закрыты).
...
### 2026-08-16 — Гарантия доставки: ретраи, очередь досылки, catch-up, recovery, crash-тревога

**Problem:** тревога «ЗАКРЫТ — ВО» 16.08 в 14:59 дошла только до 2 из 4
получателей (мигающая блокировка ТСПУ), недоставленное терялось молча.

**Что:** раздел «Гарантия доставки» выше; новые `core/open_check_pending.py` и
`tests/test_open_check_delivery.py` (36 тестов); `_send_with_retries`,
`send_crash_alarm`, параметры `note`/`queue_failures` в `run_check`/`send_report`
(open_check_bot.py); `_catch_up_if_missed`, `_resend_loop`,
`_recover_interrupted_run`, жизненный цикл lock-а `running`→`done`
(open_check_scheduler.py); `run-now` без постановки в очередь
(routes/open_check.py). Первая версия прошла адверсариальное ревью
(4 линзы x перекрёстная проверка): 13 подтверждённых находок — главные две
(потеря тревоги из-за лока очереди во время сетевых отправок; тихая потеря дня
при SIGTERM посреди прогона) закрыты клеймом и recovery. Разбор инцидента —
