...
### Виджеты рейла: «Нагрузка» и «Покрытие» (редактор И просмотр)

Оба виджета общие для `/schedule/edit` и `/schedule` — рендер один
(`Schedule.renderLoad` / `renderCoverage` в `common.js`), данные — один
**money-free** эндпоинт `GET /api/schedule/widgets/<y>/<m>` (без iiko, без рублей).
На просмотре виджеты read-only — у барменов финансов на странице нет.
...
**Покрытие по дням недели** (`compute_coverage_by_dow`) — совпадает ли штат с
нагрузкой по дням. Для каждого DOW: полоса = **относительный спрос** (среднедневной
план дня как доля от самого нагруженного DOW) и **число смен/день**. Самый
недозакрытый под спрос день (coverage = доля-штата / доля-спроса < 1) подсвечен
янтарным + строка-вывод. **Money-free:** рубли считаются на сервере, наружу уходят
только доли (`demand` 0..1) — поэтому виджет безопасен на странице барменов.

Финансовой «Сводки месяца» на странице графика нет — выручка живёт на дашборде
(убрана 2026-06-25). Старый роут `/api/schedule/summary` (считал план/факт + iiko-
часы) фронтом больше не используется — оба виджета берут данные из `/widgets`.
...
| GET | `/api/schedule/widgets/<year>/<month>` | данные виджетов (нагрузка + покрытие по DOW), money-free, без iiko — для редактора и просмотра |
