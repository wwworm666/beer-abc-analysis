...
### Нагрузка по сотрудникам (правый рейл редактора)

Сканируемая таблица по людям за месяц (группировка по стабильному `employee_id`):
- **Смен / 15** — число смен против нормы `SHIFT_NORM=15` (`core/schedule_plans.py`,
  то же значение, что в KPI-коэффициенте). Цвет: ≥нормы — зелёный (выполнил/
  перевыполнил), <60% нормы — янтарный (недобор), иначе обычный.
- **Часов** — сумма факта часов (ручной ввод бармена; единственный источник часов
  оплаты).
- **Подряд** — макс. серия смен подряд по календарным дням (`_max_consecutive_days`);
  5+ подряд подсвечивается красным — сигнал переработки, стоит дать отдых.
- **Без** — прошедшие смены без введённого факта («кто забыл проставить часы»).

Финансовой «Сводки месяца» (план/факт/выполнение) на странице графика нет — это
выручка, её место на дашборде (убрана 2026-06-25). Колонка «Часов (iiko)» убрана
ранее (после перехода оплаты на часы из графика). Расчёт iiko-часов
(`match_iiko_hours`, `_fetch_month_iiko_hours`) в роуте сводки больше не вызывается.
