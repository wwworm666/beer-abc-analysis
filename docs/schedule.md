...
- `roles` — бармен / второй бармен (роль «стажёр» убрана 2026-06-24, владелец:
  только две роли); `color` используется в UI чипов; `rate_per_hour` (v5) —
  ставка за час для расчёта ЗП (DEFAULT 300).
...
- **Контрольная колонка «Часов (iiko)»** в нагрузке: расчётные часы из
  кассовых смен (закрытие минус открытие,
  `IikoAPI.get_employee_metrics_from_shifts` — тот же источник, что
  авточасы на странице расчёта ЗП). Матчинг ФИО iiko с именами реестра —
  `core/schedule_plans.py: match_iiko_hours` (normalize/subset, как в
  `routes/employee.py`). Служит для сверки с ручным фактом, не заменяет его.
