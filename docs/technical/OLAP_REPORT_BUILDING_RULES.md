...
В проекте задействованы только `SALES` (большинство запросов) и `TRANSACTIONS` (три запроса: номенклатура `_get_nomenclature_via_olap` (обёртка `get_nomenclature_olap_only`), проводки кегов `get_draft_writeoff_report` и проводки фасовки `get_packaging_writeoff_report` — последние два через общий строитель `_transactions_report`) (сверка: `core/olap_reports.py`, `reportType` в `_get_nomenclature_via_olap` и `_transactions_report` — TRANSACTIONS; остальные — SALES). `DELIVERIES`/`STOCK` не используются (у заведения нет доставки).
