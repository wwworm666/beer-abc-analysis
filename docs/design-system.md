...
### 2026-09-05 — Раскрытие карточки панелью под рядом; шеврон в кружке

**Что:** раскрываемая карточка сетки больше не растёт внутри ячейки — детали
рисуются панелью на всю ширину под рядом, сетка неподвижна. Кликабельность
карточки показывают hover (акцентная рамка + тень) и шеврон-SVG в кружке.

**Правило:** контент, который появляется по клику внутри ячейки сетки, не
должен менять размеры сетки — он живёт панелью под рядом (или, на телефоне,
под парой). Маленький символ-стрелка в углу как единственный признак
кликабельности не читается — нужны hover-состояние и заметный шеврон.
