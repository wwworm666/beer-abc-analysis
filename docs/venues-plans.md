...
Вручную вводятся 9 метрик: `revenue`, `averageCheck`, `draftShare`, `packagedShare`,
`markupDraft`, `markupPackaged`, `markupKitchen`, `loyaltyWriteoffs`, `tapActivity`.

Остальные 7 считаются автоматически (`PlansViewer.autoCalculateDerivedMetrics` /
`updateSharesSum`) по тем же формулам, что и факт в
...
5. Прочее — списания баллов, активность кранов *(ввод)*
