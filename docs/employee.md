...
### Ручные корректировки ЗП (с 2026-07-30)

Пять категорий, как в эталонной таблице владельца (`SALARY_ADJUSTMENT_CATEGORIES`,
`core/shifts_manager.py`):

| Категория | Код | Знак |
|-----------|-----|------|
| Отпуск | `vacation` | + |
| Доп доход | `extra_income` | + |
| Вычет инвент | `deduction_inventory` | − |
| Вычет дисциплина (ручной) | `deduction_discipline` | − |
| Доп вычет | `deduction_other` | − |

- Хранение: `shifts.db`, таблица `salary_adjustments` (схема **v8**), одна строка
  на `месяц + имя сотрудника + категорию` (UPSERT). Суммы в рублях, хранятся
  **положительными** — знак задаёт категория. Сумма 0 без заметки = строка
  удаляется.
- Месяц корректировок = **месяц начала выбранного периода** (`date_from[:7]`).
- Ввод — прямо в карточке сотрудника на `/salary`; сохранение по change/blur,
  один POST на поле. Итог карточки и сводка пересчитываются на лету.
- Ключ — имя сотрудника, как в мёрже страницы; при чтении корректировка
  подвязывается через `namesMatch`, при записи используется исходный ключ БД
  (`adjKey`), чтобы не плодить варианты имени.
- Каждое изменение суммы пишется в журнал графика (`schedule_audit`, action
  `salary_adjustment`, entity_date = 1-е число месяца) — видно в истории
  `/schedule`, как и смена ставки роли.
- API: `GET /api/salary/adjustments?month=YYYY-MM`,
  `POST /api/salary/adjustments {month, employee_name, category, amount, note?}`
  (`routes/salary.py`; amount 0..1 000 000). Тесты —
  `tests/test_salary_adjustments.py`.

Итог сотрудника на странице:

```
totalSalary = премия_за_план + передача_смены − штраф_опозданий + KPI
            + оплата_часов + (отпуск + доп_доход)
            − (вычет_инвент + вычет_дисциплина_ручной + доп_вычет)
```

Авто-штраф за опоздания и ручной «Вычет дисциплина» — **разные строки** на
странице; в Excel-экспорте они складываются в одну строку «Вычет дисциплина».
