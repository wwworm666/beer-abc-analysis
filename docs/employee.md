...
- **Хранение**: `shifts.db`, таблица `handover_cash_penalties` (схема **v9**),
  одна строка на `(date, employee_name)`; имя — как в расчёте бонусов (штраф
  ставится из карточки, поэтому совпадает с iiko-именем).
...
- **API**: `POST /api/salary/handover-penalty {date, employee_name, penalized,
  note?}` (`routes/salary.py`); в ответе `bonus-calculate` по дням —
