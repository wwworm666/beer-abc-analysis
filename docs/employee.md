...
| Премия за передачу смены | `=F5*$E$9-500` | смены × тариф − неоплаченные дни |
| Такси за смены расчет. | `=F5*$E$15` | смены × тариф |
| Такси разница | `=F15+F16-F17` | расчёт + мосты − оф. |
| ИТОГО БАРМЕН | `=SUM(F6:F14)-F19-F20-F21+F18` | блок начислений − вычеты + такси |
...
| | Кнопка «Экспорт Google» | Ночная выгрузка 04:00 |
|---|---|---|
| Что делает | создаёт **новую отдельную таблицу** | пишет в таблицу бухгалтерии `SALARY_SHEET_ID` |
| Вкладка | «июль2026» в новом файле | «**Июль_2026_Автоматическая**» |
| Данные | из расчёта, открытого на странице | собираются на сервере |
| Перезапись | нечего перезаписывать | свою вкладку целиком, каждую ночь |
