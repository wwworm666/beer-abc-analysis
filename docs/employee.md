...
**Формул в файле нет — все строки посчитаны сервером и записаны числами.**
openpyxl не сохраняет кэш значений формул, из-за чего формульные ячейки
выглядели пустыми в предпросмотре Google Drive и защищённом режиме Excel
(жалоба владельца 2026-07-31). Следствие: после ручного вписывания «мостов»
бухгалтер сводит разницу в своей таблице (файл сам не пересчитается).
Контроль отсутствия формул — тест `test_no_formulas_anywhere`.
...
- Тесты: `test_day_shifts_for_taxi` (`tests/test_salary_hours.py`),
  `test_taxi_and_totals_are_values` (сверка ИТОГО с эталоном владельца:
  Юреня = 48 109) и `test_no_formulas_anywhere`
  (`tests/test_salary_export.py`).
...
- Книгу строит `build_salary_workbook()` (openpyxl). Живые формулы Excel:
  - «Такси за смены расчет.» `= смены × тариф` (тариф 700 в ячейке — правится
    в файле, всё пересчитается);
  - «Такси разница» `= расчёт + мосты − оф.` — «мосты» и «Такси за смены оф.»
    дозаполняются вручную в файле;
  - «ИТОГО БАРМЕН» `= оплата_часов + отпуск + премии + KPI + доп_доход +
    такси_разница − вычеты`; колонка ИТОГО — `SUM` по строке.
- Строки «Отпуск», «Доп доход», «Вычет инвент», «Доп вычет» выгружаются
  **пустыми** — владелец заполняет их в Excel (с 2026-07-31 приложение эти
  суммы не ведёт); «Вычет дисциплина» = авто-штраф за опоздания. Ручной штраф
  кассы уже входит в строку «Премия за приемку-передачу смены».
- «Количество смен» — смены из графика за период (`shifts_with_fact +
  shifts_without_fact`); фоллбэк — кассовые смены iiko, если сотрудника нет
  в графике.
