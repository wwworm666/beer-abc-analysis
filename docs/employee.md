...
|---|---|---|
| Ставка по часам / 2-й в смене | `=F3*$E$7` | часы × тариф |
| Премия за передачу смены | `=F6*$E$10` | оплаченные дни × тариф |
| Такси за смены расчет. | `=F5*$E$16` | дневные смены × тариф |
| Такси разница | `=F16+F17-F18` | расчёт + мосты − оф. (справочно) |
| ИТОГО БАРМЕН | `=SUM(F7:F15)-F20-F21-F22+F16+F17` | начисления − вычеты + такси целиком |
| Колонка ИТОГО | `=SUM(F7:N7)` | сумма по строке |

> **Строка «Оплаченных дней передачи смены» (с 2026-08-06).** Премия за передачу смены
> считается от своей базы — оплаченных дней (`handover_paid_days`), — а не от
> «Количества смен»: там дневные смены графика (база такси), и числа не
> совпадают. Пока премия была формулой от чужой базы, разницу приходилось гасить
> константой, и в файле появлялся вычет «−2000» там, где штрафов на 2 500, плюс
> фантомные «−500» у тех, у кого штрафов нет вовсе (жалоба владельца). Новая
> строка стоит среди счётчиков (строка 6), а не в блоке начислений — иначе
> счётчик дней попал бы в `SUM` строки ИТОГО БАРМЕН.
