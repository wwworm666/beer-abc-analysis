...
**Что выгружается ночью:** текущий месяц всегда; предыдущий — пока число
месяца <= `SALARY_SYNC_PREV_UNTIL_DAY` (по умолчанию 10), чтобы закрытый месяц
дособирался по дедлайнам самой таблицы («05 число», «10 число»).
