...
Компенсация такси: **700 ₽ × полноценная дневная смена** из графика.
...
- Тариф 700 — константа `TAXI_RATE_PER_SHIFT` (`core/salary_export.py`,
  единственное место значения); страница получает его из ответа
  `GET /api/schedule/hours-by-role` (`taxi_rate` + `day_shifts` по
  сотруднику) и показывает строкой «Такси — N дн. смен x 700» в карточке;
  входит в итог сотрудника и сводку.
- **В экспорте**: «Количество смен» = те же дневные смены; «Такси за смены
  расчет.» — живая формула `смены × тариф`. «Мосты» и «Такси за смены оф.»
  заполняются в Excel, «Такси разница» и ИТОГО пересчитаются сами (пока
  «оф.» пусто, разница равна расчёту — как на странице).
- Тест: `test_day_shifts_for_taxi` (`tests/test_salary_hours.py`).
