...
### Экспорт в Google Таблицу (с 2026-08-01)

Вторая кнопка на `/salary` — **«Экспорт Google»**: тот же расчёт записывается
**вкладкой в Google Таблицу** (имя вкладки — месяц, «июль2026», как листы в
таблице бухгалтерии).

- Зачем: бухгалтерия работает в Таблицах (эталонный файл сам выгружен
  оттуда), и Таблицы считают формулы на сервере — проблемы «формульные ячейки
  пусты в предпросмотре» здесь нет вообще.
- **Раскладка общая с .xlsx** — `core/salary_layout.py`. Строки, формулы,
  тарифы и цвета описаны один раз, рендереров два
  (`salary_export.py` -> openpyxl, `salary_gsheet.py` -> Sheets API). Паритет
  закреплён тестом `test_parity_with_xlsx`: сверка ячейка в ячейку, так что
  правка в обход раскладки сломает тест.
- **Существующая вкладка молча не перезаписывается** (409 -> подтверждение на
  странице): бухгалтер вносит в неё руками «мосты», «Отпуск», «Доп доход» и
  вычеты — приложение эти строки не ведёт и восстановить не сможет.
- Авторизация — сервис-аккаунт Google Cloud (API-ключ на запись не работает).
  ...
  «не настроено» и ничего не ломает; библиотеки Google импортируются лениво,
  поэтому приложение поднимается и без них.
- Переменные: `GOOGLE_SA_JSON` (путь к ключу), `SALARY_SHEET_ID` (целевая
  таблица; пусто — создаётся новая), `SALARY_SHEET_FOLDER_ID` (папка Drive).
- API: `POST /api/salary/export-gsheet` (тот же payload + `overwrite`), ответы
  200 `{url, tab}` / 409 `{exists, tab, url}` / 503 не настроено. Каждый
  экспорт пишется в журнал графика (`salary_gsheet_export`).
- Тесты — `tests/test_salary_gsheet.py` (12 шт., сеть не дёргается: проверяются
  запросы Sheets API, которые модуль отправил бы).
