...
факт выше минимума — 0. На экранах такой показатель подписан «Максимум», а не «Минимум»
(«минимум 3 опоздания» противоречил бы смыслу), и пересчёт на смены показывается по порогу:
«Порог зависит от смен: за норму 15 смен — 3 шт, за 5 смен — не больше 1 шт». До 2026-09-06 защита «факт ниже минимума — ноль» стояла перед формулой
...
- **факт** = значение за период / кассовые смены за период (`metrics['shifts_count']` из iiko —
  «мы считаем смены по кассам»; без него — дни смен). В ответе: `fact` — за смену, плюс
  `fact_raw`, `shifts_divisor`, `unit` («шт/смену»), `decimals`;
- **цель и минимум** хранятся за одну кассовую смену (`per_shift: true`), с точностью
  `PER_SHIFT_STORE_DECIMALS = 4` — при 10/15 = 0.6667 обратный пересчёт даёт ровно введённые 10;
...
**«За смену» — техническая единица, на экран она не идёт.** Владелец 2026-09-06: «я захожу внутрь
сотрудника — он сделал 0,75, нужно было 1,67. Чего?» Дробь на человека не действует, поэтому всюду
показывается ЦЕЛЬ ЗА ЕГО СМЕНЫ в обычных штуках, а вводится цель за норму смен:

| Где | Что видно |
|---|---|
| Панель «KPI» на `/salary` | «Факт 8 шт · Цель 10 шт · Минимум 3,3 шт» + строка «за норму 15 смен — 30 шт, за 5 смен — 10 шт» (период выбирают руками) |
| Карточка KPI на `/me` | цель на ВЕСЬ месяц по графику смен: «Сделано 2 шт · Цель 6 шт (на месяц · 12 смен по графику) · осталось 4 шт»; множитель и премия — по темпу за отработанное (см. [me.md](me.md)) |
| Таблица по точкам | цель точки за смены сотрудника именно на ней (5 смен на Крем → 10 шт) |
| Редактор целей | обычные 30 / 10 за норму смен, бейдж «за 15 смен», чекбокс «зависит от смен» |
| `/goals` | «30 шт», подпись «за 15 смен — ваша цель пересчитывается на ваши смены» |

Числа за период считает сервер (`target_period`, `min_period`, `period_decimals` в ответе;
у точек — `location_targets[loc].target_period`), фронты только форматируют: дробный знак
появляется, лишь когда он есть по существу (минимум 3,3 шт), у рублей не появляется никогда.
Редактор хранит канон «за смену», а показывает `значение × норма` — поэтому переключение чекбокса
не меняет введённые числа на экране, меняется только смысл (фиксированный порог или доля от смен).

Пример: цель 2 карты за смену, минимум 0,67; два KPI, база 7 500 ₽.

| Бармен | Касс. смен | Его цель | Факт | Множитель | Коэфф. | Премия по KPI |
|---|---|---|---|---|---|---|
| A | 5 | 10 шт (мин 3,3) | 8 шт | 0,70 | 0,33 | 1 732 ₽ |
| B | 25 | 50 шт (мин 16,7) | 40 шт | 0,70 | 1,67 | 8 767 ₽ |

Раньше при месячной цели 30 / мин 10 одним числом на всех: A — множитель 0 и 0 ₽, B — множитель 1,5
и 12 525 ₽ при одинаковой работе (1,6 карты за смену у обоих).
  ...
  "months": {
    "2026-08": {
      "kpi_config": {
        ...
        "kpi2": {"metric": "loyalty_cards_count", "name": "Новые карты лояльности (шт)", "per_shift": true}
      },
      "Кременчугская": {
        "kpi1": {"target": 18.0, "min": 13.0},
        "kpi2": {"target": 2.0, "min": 0.6667}
      },
      ...
    }
  },
...
`per_shift`, `fact_raw`, `shifts_divisor`, `target_period`, `min_period`, `period_decimals`, `unit`,
`decimals`, `no_targets`), `templates/goals.html` (цель за норму смен).
...
### 2026-09-09 (2) — Показатели «меньше — лучше» подписаны как максимум

Прогон страниц глазами бармена: у опозданий плитка называлась «Минимум 1 шт», хотя это
потолок, а пояснение пересчитывало на смены цель (всегда 0) вместо порога. Исправлено на
`/salary`, `/me` и `/goals`: подпись «Максимум» и колонка «Макс ТТ» при `min > target`,
пересчёт «Порог зависит от смен: за норму 15 смен — 3 шт, за 5 смен — не больше 1 шт»,
у нулевой цели убрана бессмысленная подпись «за N смен»; на `/goals` — «максимум 3 шт —
выше премии нет» вместо «минимум ... — выше премии нет».
...
Владелец о первой версии: «математика верная, но визуально непонятно: он сделал 0,75, нужно было
1,67 — чего?» Единица «за смену» осталась внутри формулы, а на экранах цель считается на смены
конкретного человека и показывается в штуках: «сделал 8 из 10» вместо «0,80 из 2,00». В ответ
расчёта добавлены `target_period`, `min_period`, `period_decimals` (и те же поля по точкам в
`location_targets`); панель «KPI», `/me` и `/goals` показывают их, дробное «за смену» осталось
только в пояснительной строке в форме «за норму 15 смен — 30 шт, за ваши 5 — 10 шт». Редактор целей
принимает обычные числа за норму смен (30 / 10) и хранит канон «за смену» с точностью 4 знака —
без неё 10/15 = 0,67 возвращалось как 10,05. Автоназвание метрики больше не тащит «/смену»,
чекбокс называется «зависит от смен», у строки сетки бейдж «за 15 смен».
