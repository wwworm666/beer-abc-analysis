...
**Excel-экспорт — вид бухгалтера (на сайте не показывается):**

```
расчёт  = смены × тариф                  (7 700 при 11 сменах)
оф.     = 15 × тариф = 10 500            (фикс — правило владельца; 0 без дневных смен)
мосты   = ручная надбавка бухгалтера     (единственная ручная строка, пустая)
разница = расчёт + мосты − оф.           (идёт в ИТОГО файла: доплата/удержание)
```

Строки «расчёт» и «разница» — **живые формулы** (с 2026-08-01), поэтому
вписанные бухгалтером «мосты» сразу меняют разницу и ИТОГО БАРМЕН прямо в
...
- Константы — единственное место значений в `core/salary_export.py`:
  ...
  по сотруднику).
- Из-за оф./мостов **итог страницы отличается от ИТОГО файла** на зачёт
  официальной оплаты — это осознанно: бармен видит фактическое такси,
  бухгалтер сводит в Excel.
...
| ИТОГО БАРМЕН | `=SUM(F7:F15)-F20-F21-F22+F19` | блок начислений − вычеты + такси |
