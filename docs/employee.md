...
В ЗП идёт **разница** между расчётным такси и официальной оплатой:

```
расчёт  = day_shifts × 700          (полноценные дневные смены из графика)
оф.     = 15 × 700 = 10 500         (фикс — правило владельца; 0, если дневных смен нет)
разница = расчёт − оф.              (+ доплата / − удержание; входит в итог ЗП)
```
...
- Константы — единственное место значений в `core/salary_export.py`:
  `TAXI_RATE_PER_SHIFT = 700`, `TAXI_OFFICIAL_SHIFTS = 15`; страница получает
  их из `GET /api/schedule/hours-by-role` (`taxi_rate`,
  `taxi_official_shifts`, `day_shifts` по сотруднику). В карточке — строка
  «Такси разница» со знаком (подробности в title), в сводке — карточка
  «Такси разница».
- **«Мосты»** — ручная надбавка бухгалтера, ведётся только в Excel (на
  странице её нет; из-за неё итог страницы может отличаться от файла после
  ручного заполнения).
- **В экспорте**: «Количество смен» = те же дневные смены; «Такси за смены
  расчет.» — формула `смены × тариф`; «Такси за смены оф.» — формула
  `IF(смены>0; 15 × тариф; 0)`; «Такси разница» и ИТОГО считаются сами.
- Тесты: `test_day_shifts_for_taxi` (`tests/test_salary_hours.py`),
  `test_formulas` (`tests/test_salary_export.py`).
