...
### Неполная выдача `/corporation/groups` → ложное «бар закрыт» (open-check)

**Problem:** 2026-06-17 open-check-бот слал «ЗАКРЫТ — Варш», хотя кассовая смена
Варшавской была открыта (касса №50, pos `f4099015`). Прямой `curl` к iiko с того
же VPS стабильно (8/8) возвращал точку Варшавской, локально код маппил её верно —
а контейнер бота упорно её не видел даже после рестарта (сброс кэша) и истечения
TTL кэша групп.

**Cause:** `get_groups_with_pos()` (`/corporation/groups`) периодически отдаёт
НЕПОЛНЫЙ список точек продаж — узлы iiko рассинхронизированы после перезаведения
кассы (Варшавской недавно сменили pos_id). Если в ответе нет точки, её `pos_id`
отсутствует в `pos_map`, и в `check_bars_state` срабатывало `if not pos_name:
continue` — открытая смена **молча отбрасывалась**, бар ложно числился закрытым.
Усугублялось 10-минутным кэшем `_GROUPS_CACHE`: одна неполная выдача → до 10 мин
ложных тревог. (`unknown_pos` при этом ПУСТ — туда попадают только смены с
известным именем, но без маппинга, а тут имени не было вовсе.)

**Solution:** Резерв `pointOfSaleId -> venue_key`, который НИКОГДА не сжимается
(`core/open_check_bot.py`):
- базовый seed из текущих pos_id 4 баров (`_SEED_POS_MAP`);
- выученные привязки персистятся на диск (`open_check_pos_map.json`): каждая
  ПОЛНАЯ выдача дополняет карту;
- если у открытой смены `pos_id` нет в живой выдаче — берём из этого резерва
  (с `log.warning`), а не выкидываем смену. Настоящий silent-skip заменён на лог.

При смене pos_id бар переучивается с первой же полной выдачи; устаревшая
seed-запись просто перестаёт совпадать с реальными сменами и ни на что не влияет.
