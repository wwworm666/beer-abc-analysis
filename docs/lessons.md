...
## Открытая вкладка браузера откатывает деплой: порядок колонок пришёл от клиента

**Problem:** на следующий день после деплоя алфавитной сортировки июльская вкладка Google
снова оказалась в старом порядке — хотя накануне я её исправил и проверил. Августовская
осталась правильной. Выглядело как «сортировка не работает» или «ночная выгрузка пишет
мусор».

**Cause:** журнал графика показал `salary_gsheet_export` в 21:54 UTC — кнопку «Обновить
Google» нажали со страницы `/salary`, открытой ДО деплоя. Service worker сетевой-первый,
так что перезагрузка отдала бы свежий JS, но вкладку никто не перезагружал: старый код
страницы сортировал по сумме ЗП. А сервер писал колонки РОВНО в порядке payload — то есть
экспорт доверял порядку, присланному клиентом, и устаревшая вкладка затёрла исправленный
лист. Августовскую никто кнопкой не трогал, поэтому она уцелела — эта асимметрия и была
главной уликой.

**Solution:** порядок колонок задаёт сервер. `build_sheet` (`core/salary_layout.py`)
сортирует сотрудников сам — это общий чокпоинт .xlsx, кнопки и ночной выгрузки, поэтому
клиент любой свежести не может испортить лист. Сортировка на странице осталась: она про
порядок карточек на экране.

**Правило на будущее:** если правка меняет ФОРМУ данных, которые клиент присылает на
запись, серверу нельзя оставлять эту форму на усмотрение клиента — у пользователей
неделями висят открытые вкладки со старым кодом. Проверять такие случаи стоит по журналу
(`schedule_audit`, `action LIKE '%gsheet%'`): он показывает, кто и когда писал.

---

## Google-вкладка — снимок, а не живой вид: деплой её не обновляет

**Problem:** сразу после деплоя алфавитной сортировки владелец открыл Google Таблицу и
сказал, что бармены не по алфавиту. На странице `/salary` порядок был правильный, тесты
зелёные, серверный `name_sort_key` в контейнере на месте — расхождение выглядело как
непонятный баг сортировки в Sheets-рендерере.

**Cause:** вкладка «..._Автоматическая» — снимок последнего прогона выгрузки, а не
представление расчёта. Пишется она только ночью в 04:00 и по кнопке «Обновить Google»;
деплой её не трогает. Владелец смотрел на содержимое, записанное в 04:00, то есть
несколькими правками ранее. Явный признак был прямо в шапке: в августовской вкладке стоял
«Алексей Стажер», хотя переименование в реестре мы распространили ещё до этого деплоя.

**Solution:** после деплоя правки, меняющей содержимое листа, вкладку надо перезаписать —
кнопкой «Обновить Google» на странице или на проде тем же кодом, что ночная выгрузка:
`build_payload_for_month(app, month)` + `sync_to_master(payload)` за нужные месяцы.
Проверка — обратное чтение шапки листа (`values().get(range="'вкладка'!A2:BZ2")`),
сотрудники начинаются с колонки F.

Два побочных правила из этого же разбора:
- **Не импортировать `app` в разовых скриптах на проде.** `import app` поднимает
  планировщики и Telegram long-polling, а второй `getUpdates` перетягивает апдейты у
  боевого поллера. `build_payload_for_month` использует приложение только как контекст
  запроса — хватает пустого `Flask('resync')`.
- **Ручные вкладки бухгалтера трогать нельзя** («июль 2026» и старые месяцы): там свой
  порядок колонок, свои имена («Дарья Концова») и строки, которых приложение не знает.
  «Не по алфавиту» в них — норма, а не симптом.

---

## Имя как ключ: переименование в iiko разорвало сотрудника на двух в расчёте ЗП

**Problem:** владелец переименовал бармена в iiko («Алексей Стажер» -> «Алексей Марченко»)
и увидел на `/salary` и в графике ДВУХ разных людей. В июльском экспорте это выглядело так:
у «Алексея Стажера» 5 дневных смен, часы и такси, но премии и KPI = 0; у «Алексея Марченко»
1 кассовая смена и премия 500, но ни часов, ни такси. ЗП одного человека разъехалась на две
колонки, и обе половины неверны. Заодно в файле у него появился фантомный вычет «−2000»,
похожий на штраф кассы.

**Cause:** в базе никакого дубля не было — одна строка реестра, один `employee_id`
(`4b83e5be-…`), 20 смен. Расщепление рождалось в МЁРЖЕ страницы: три источника (bonus, KPI,
часы из графика) сводились по СЛОВАМ ИМЕНИ (`namesMatch` — одно множество слов подмножество
другого). График хранит имя снимком рядом со стабильным id, iiko отдаёт актуальное — и
`{алексей, марченко}` с `{алексей, стажер}` не пересекаются. Соседний случай уцелел
случайно: `Станислав Колганов` ⊂ `Станислав Колганов Стажер` — подмножество сматчилось.
Переименование, меняющее фамилию, ломает матч всегда. Ручной штраф кассы хранился по имени
и точным совпадением искался тоже по имени — то есть переименование молча осиротило бы
уже проставленные штрафы.

**Solution:** два уровня.
1. Разово — синхронизация реестра (`sync_employees`, кнопка «Обновить из iiko» на
   `/schedule`) распространяет переименование по `employee_id` на снимки имён: 48 смен
   переименовано, человек склеился обратно. Механизм существовал, но запускается только
   руками — до клика ЗП считается неверно.
2. Структурно — личность больше не имя. `bonus-calculate`, `kpi-calculate` и
   `hours-by-role` отдают `employee_id`; мёрж (и в `templates/bonus.html`, и в
   `core/salary_payload.py`) сначала матчит по id, имя осталось фоллбэком для смен без id и
   людей не из iiko. Штраф кассы получил `employee_id` (схема v10) с бэкофиллом по реестру.

**Грабля внутри решения:** первая версия матча «сначала id, иначе имя» оказалась ОПАСНЕЕ
исходной — если у сотрудника нет пары по id, фоллбэк по имени мог подцепить запись с ДРУГИМ
id (часы одного бармена в ЗП другого). Правильное правило: если обе стороны знают id и они
разные — это разные люди, и похожее имя не аргумент; имя связывает только там, где id
отсутствует хотя бы с одной стороны. Поймано тестом `test_merge_id_wins_over_name`.

**Общий урок:** отображаемое имя из внешней системы — не ключ, а изменяемый атрибут. Если
рядом есть стабильный id, сводить надо по нему, а fuzzy-матч имён держать только как мост
для данных без id — и обязательно запрещать ему связывать записи с заведомо разными id.

---

## Планировщик, дёргающий свой же HTTP-эндпоинт, ловит auth-гейт (401)

**Problem:** данные ЧЗ на `/expiration` и `/stocks` молча перестали обновляться после
26.06.2026 — дата «Данные ЧЗ» застыла. Ночной авторефреш каждую ночь срабатывал (суточные
lock-файлы `.chz_refresh_lock_<date>` создавались), но `chz_stock.json` не обновлялся, а
`refresh.log` не писался — то есть подпроцесс `remote_exec run search-stock` вообще не
стартовал.

**Cause:** `core/chz_scheduler.py` слал `POST http://127.0.0.1:10000/api/chz/refresh`
обычным `urllib` — без cookie сессии. Глобальный auth-гейт (`core/auth_guard.py`, добавлен
23.06.2026) закрывает всё, кроме `PUBLIC_ENDPOINTS`, и на любой `/api/*` без входа отдаёт
`401`. Внутренний запрос планировщика попадал под тот же гейт → 401 → refresh не запускался.
Классика: сервис ходит сам к себе через HTTP и упирается в собственную авторизацию. Ручная
кнопка при этом работала (у пользователя есть сессия) — потому последнее обновление 26.06
и было ручным.

**Solution:** тело refresh вынесено в функцию `start_chz_refresh()` в `routes/stocks.py`;
HTTP-эндпоинт стал тонкой обёрткой, а планировщик зовёт функцию НАПРЯМУЮ (ленивый импорт),
минуя гейт. Cross-worker file-lock общий, так что кнопка и планировщик по-прежнему не
запускают refresh параллельно. Общий урок: внутренние триггеры (cron/scheduler) не должны
ходить в свои же защищённые HTTP-роуты — выноси логику в функцию и зови из процесса, либо
делай эндпоинт публичным с проверкой `remote_addr==127.0.0.1`.

---

## График: read-modify-write под gunicorn 2 воркера = гонка

**Problem:** `update_revenue` (SELECT id → UPDATE/INSERT) под `threading.Lock` иногда
падал 500 на `UNIQUE(date, location_id)`, а `sync-month` рвался на середине месяца,
оставляя выручку наполовину засинканной (кривые «Ожидаемая»/«Выполнение %»).

**Cause:** `threading.Lock` сериализует только потоки ОДНОГО процесса. На проде 2 воркера
gunicorn — два процесса, каждый со своим Lock. При параллельном вызове на одну дату/точку
оба видят «строки нет» и оба идут в INSERT → IntegrityError у второго (логический TOCTOU
между процессами; WAL/busy_timeout не спасают — конфликт не на уровне блокировки файла).

**Solution:** многошаговый RMW в БД — атомарным SQL. Для выручки:
`INSERT … ON CONFLICT(date, location_id) DO UPDATE SET … COALESCE(?, col)`. Один стейтмент —
нет TOCTOU, идемпотентно (повторный синк не падает). Тот же приём уже был в `save_wish`.
Для других многошаговых операций между воркерами — cross-worker file_lock (как `json_store`),
а не `threading.Lock`.
...
**Problem:** смену можно было создать с несуществующими `location_id`/`role_id`. Такая
orphan-смена пропадала из сетки (её прятал INNER JOIN в `get_shifts_for_month`), занижала
часы и не попадала в журнал (после INSERT роут звал `get_shift` с INNER JOIN → None → аудит
тихо пропускался).

**Cause:** SQLite не применяет объявленные в схеме FOREIGN KEY, пока в КАЖДОМ соединении не
выполнить `PRAGMA foreign_keys=ON` (по умолчанию off ради совместимости).

**Solution:** `PRAGMA foreign_keys=ON` в `_get_connection` (рядом с WAL/busy_timeout). Плюс
валидация id в роутах и `except sqlite3.IntegrityError → 400`. Внимание при первом деплое:
если в проде уже есть orphan-смены, UPDATE такой строки теперь упрётся в FK — но фронт всегда
шлёт реальные id из тулбара, так что в норме orphans нет.

---

## График: 0 — это значение, а не «пусто» (факт часов/выручки)

**Problem:** в `.ics` смена с введённым фактом «0:00» показывалась как 9-часовая «по плану».

**Cause:** проверки вида `if fact:` и тернарники трактуют валидный `0` как falsy = «факта нет»,
хотя бэкенд допускает `fact_minutes` 0..1440, и `0` означает «бармен ввёл ноль».

**Solution:** различать «нет значения» и «ноль» через `is not None`
(`dur = int(fact) if fact is not None else PLAN_MINUTES…`). Общий урок для детерминированных
расчётов: `0`, пустая строка и `None` — разные состояния, не схлопывать их в один `if x`.

---

## График: один порог «вечер» на всё приложение

**Problem:** смена 16:00–17:59 рисовалась как «вечер», «Покрытие точек» показывало ложные
красные дыры, а подписи в легенде/.ics противоречили сетке.

**Cause:** три критерия «вечера» в разных местах: `isEvening` в JS (`>= '16:00'`), пресет
кисти и .ics (`18:00`), легенда («с 17:00»).

**Solution:** единый детерминированный порог `start_time >= '18:00'` (как пресет «Вечер» и
.ics) + роль «второй …»; легенда приведена к нему. Любую сегментацию день/вечер считать ровно
по одному правилу во всех модулях.
