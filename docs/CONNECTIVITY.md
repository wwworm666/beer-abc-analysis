...
- **Имя:** `DESKTOP-E0BLMGC`, пользователь `sshuser` / `<пароль sshuser, см. менеджер паролей>`
...
- Попытки логина как `Администратор` / `Administrator` с паролем `<пароль Администратора, см. менеджер паролей>` — все FAIL
...
|---|---|---|
| sshuser | <пароль sshuser, см. менеджер паролей> | OK (ключ + пароль) |
| Администратор | <пароль Администратора, см. менеджер паролей> | FAIL (Authentication failed) |
| Administrator | <пароль Администратора, см. менеджер паролей> | FAIL (Authentication failed) |
| DESKTOP-E0BLMGC\Администратор | <пароль Администратора, см. менеджер паролей> | FAIL |
| DESKTOP-E0BLMGC\Administrator | <пароль Администратора, см. менеджер паролей> | FAIL |
...
- **Пользователи:** `1` / `<пароль RDP, см. менеджер паролей>`, `Андрей` (RDP)
- **OpenSSH:** есть, но **НЕ отвечает** на порту 22 на момент 07.04.2026

### Доступ
- SSH через sshuser: FAIL — port 22 не отвечает (connect refused)
- RDP через `1` / `<пароль RDP, см. менеджер паролей>`: OK, **работает**
- RDP через `Андрей` / `<пароль RDP, см. менеджер паролей>`: FAIL (неверный пароль)
