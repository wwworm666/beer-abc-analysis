...
|---|---|---|
| `negative_stock` | `stock < 0` | «остаток −1 шт: проверить учёт в iiko, заказом не лечится» |
...
| `order` | `recommended > 0` | «нужно 12 шт (5 дн. до следующей поставки + 3 дн. запаса), есть 4, в пути 6: заказать 2 к пт 07.11» |
| `in_transit` | рекомендация 0, `on_order > 0` | «в пути 10 шт: с ним хватит до следующей поставки» |
| `enough` | остальное | «хватит на 13 дн., до следующей поставки 2 дн.» |

**Секция** (`section`): `decide` — `recommended > 0` или `stock < 0`; `idle` —
`dead`/`slow`; `ok` — хватает. Экран показывает `decide` и позиции в черновике, `ok` — по
переключателю, `idle` — в свёрнутом блоке, который никогда не исчезает.
...
reason_code, reason, section`. Сортировка: срочность, затем `days_left`. Перед расчётом доска сверяет
...
(`POST /api/orders/draft`, одна позиция — один запрос), значение подставляется из
`draft_qty` и синхронизируется между вкладками. Строка помнит бар, для которого построена
(`data-bar`), поэтому смена бара в селекте не отправит позицию не в тот бар. Таплист
...
Вкладка «К отправке»: карточки черновика по поставщикам с текстом для чата, «Скопировать
текст», «Отправлено поставщику» (черновик → заказ, ожидаемая дата по календарю поставщика),
CSV запасной кнопкой; блок «В пути» с «Приехало» и «Отменить»; история за 30 дней.
Устройство, статусы, API — в [orders.md](orders.md).
