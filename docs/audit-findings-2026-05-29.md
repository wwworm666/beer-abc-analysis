...
## Запись для docs/CHANGELOG.md (вставить вручную)

> Не добавлено в `docs/CHANGELOG.md` автоматически: на момент работы файл содержал
> параллельную незакоммиченную правку (удаление `docs/iiko-api/`). Чтобы не смешивать
> два независимых коммита — запись оставлена здесь для ручного переноса.
