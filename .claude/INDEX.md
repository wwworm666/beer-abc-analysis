...
- **2026-06-26:** График: виджет «Покрытие по дням недели» (money-free: относительный спрос из плана vs смены/день, подсветка недозакрытого дня) + оба виджета («Нагрузка» + «Покрытие») и «Пожелания» продублированы на страницу просмотра `/schedule` (read-only). Общий эндпоинт `GET /api/schedule/widgets` (без iiko/денег), рендеры вынесены в `common.js`. Идея — из deep-research по ростер-дашбордам. Адверсариал-ревью: 3 low-замечания поправлены. Док: [schedule.md](../docs/schedule.md) «Виджеты рейла».
