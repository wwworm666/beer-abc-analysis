...
# Объём порции iiko -> поле цены карточки. Объём берётся из имени блюда
# («… (0,33)», «… (1,0)») парсером DraftAnalysis. Какие из них показывать на
# карточке, решает поле vols карточки (routes/menu_editor.py, реестр VOLUMES) —
# здесь только заполнение цен.
PORTION_FIELD = {0.25: "p025", 0.33: "p033", 0.4: "p04", 0.5: "p05", 1.0: "p10"}
