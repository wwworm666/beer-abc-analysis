...
Порядок строк: часы по ролям, «Количество смен», оплата по ролям, «Отпуск»,
премия за передачу, премия за дневной план, KPI (N строк), «Доп доход», блок
такси (4 строки), вычеты (3), ИТОГО БАРМЕН. Счётчики стоят ДО блока начислений
намеренно: ИТОГО суммирует непрерывный диапазон «оплата..доп доход», и счётчик
внутри него уехал бы в деньги.
...
листе не выводимы.
    ...
    Такси расчет.        =F5*$E$15         дневные смены графика x тариф
    Такси разница        =F15+F16-F17      расчёт + мосты - оф. (справочно)
    ИТОГО БАРМЕН         =SUM(F6:F14)-F19-F20-F21+F15+F16
    Колонка ИТОГО        =SUM(F6:N6)

ИТОГО берёт такси ПОЛНОСТЬЮ (расчёт + мосты), а не разницу: сумма листа обязана
совпадать со страницей /salary. «Такси за смены оф.» (фикс 15 смен) и «Такси
разница» остались строками для бухгалтерии и в итог не входят — до 2026-08-07
итог считался от разницы, и лист был меньше сайта ровно на 10 500 у каждого.

Номера строк здесь — для 2 ролей и 3 KPI; в коде они вычисляются, а не зашиты.

Сверка, чтобы формула не соврала (формула ставится ТОЛЬКО если воспроизводит
сумму страницы, иначе пишется число — экспорт обязан совпадать с расчётом):
страница считает оплату из неокруглённых часов (`minutes/60 x ставка`), а в
ячейку часов идёт округление до сотых — на длинном месяце «часы x тариф»
расходится со страницей на рубль.

## Почему премия за передачу смены — ЧИСЛО, а не формула

Премия считается по ДНЯМ смен минус дни без сданной кассы и дни с ручным
штрафом (`routes/employee.py:_handover_premium`), а «Количество смен» в листе —
это дневные смены графика, база такси. Это разные числа, и формула
«смены x тариф» их не воспроизводит: пока она стояла, разницу гасили
константой, и в файле появлялся вычет «-2000» там, где штрафов на 2 500, плюс
фантомные «-500» у тех, у кого штрафов нет вовсе (жалоба владельца 2026-08-06).
Своя строка-счётчик базы решала это, но владелец убрал её как лишнюю
(2026-08-07): премия и так отдельная строка, а тариф 500 виден в колонке
«Тариф». Поэтому в ячейке — готовая сумма страницы, без заглушек.

## Changelog

- 2026-08-07 — строка «Оплаченных дней передачи смены» убрана (решение
  владельца), премия пишется числом; лист вернулся к строкам 3..22.
...
# «Такси за смены оф.» — официально такси всегда проводится за фиксированные
# 15 смен (15 x 700 = 10 500, правило владельца). СПРАВОЧНАЯ строка для
# бухгалтерии: в ИТОГО она не входит, потому что бармену платят по факту —
# 700 за смену (решение владельца 2026-08-07, до него итог считался от разницы
# и лист расходился со страницей ЗП на 10 500). Строка «Такси разница»
# (расчёт + мосты - оф.) осталась там же и тоже справочная; «мосты» — ручная
# надбавка бухгалтера, она идёт в ИТОГО вместе с расчётом. Если дневных смен за
# период нет, оф. не начисляется. Число фиксированное, поэтому пишется
# значением, а не формулой от тарифа — как в эталоне.
...
def build_sheet(payload: dict) -> Sheet:
    ...
    # Пишется ЧИСЛОМ (сумма страницы ЗП), а не формулой. Формуле нужна база —
    # оплаченные дни, а её нельзя взять из «Количества смен»: там дневные смены
    # графика, другое число. Отдельная строка-счётчик у листа была (2026-08-06),
    # но владелец убрал её как лишнюю: премия и так есть строкой (2026-08-07).
    # Тариф 500 остаётся в колонке «Тариф» — по нему видно, из чего сумма.
    ...
    # --- ИТОГО БАРМЕН ---
    # Непрерывный блок «оплата..доп доход» - вычеты + такси ПОЛНОСТЬЮ
    # (расчёт + мосты). Часы, смены и «такси оф.» в итог не входят.
    #
    # До 2026-08-07 в итог шла такси-РАЗНИЦА (расчёт + мосты - оф. 10 500), как
    # в ручной вкладке бухгалтерии. Из-за этого лист расходился со страницей ЗП
    # ровно на 10 500 у каждого, и владелец не мог свести суммы. Решение
    # владельца: «в таблице и на сайте должны быть одинаковые настоящие полные
    # суммы вместе с такси; платим в смену 700 руб, 10 500 — это для
    # бухгалтерии строка». Поэтому «Такси за смены оф.» и «Такси разница»
    # остались справочными строками листа, но в ИТОГО больше не участвуют.
    def total_cells(r):
        if not pay_rows:
            return [None] * n_emp
        return [Formula(f"=SUM({L}{pay_rows[0]}:{L}{extra_income_row})"
                        + ''.join(f"-{L}{d}" for d in ded_rows)
                        + f"+{L}{taxi_calc_row}+{L}{bridges_row}")
                for L in emp_letters]
