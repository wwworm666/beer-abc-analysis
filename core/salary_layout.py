...
- *премия за передачу смены*: считается по кассовым сменам iiko, а строка
  «Количество смен» — по дневным сменам графика; формула ставится, только если
  разница кратна тарифу.
...
def build_sheet(payload: dict) -> Sheet:
    ...
    def handover_cell(e, r, letter):
        """Формула, если она сходится с «Количеством смен» листа.
        ...
        «=F5*$E$9-500»; когда нет — премию числом.
        """
