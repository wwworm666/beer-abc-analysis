"""
Кассовый регистр за период — рабочий стол бухгалтера в разделе «Касса за месяц»
на странице ЗП (/salary).

## Что это

Одна таблица со ВСЕМИ сменами периода и их кассой: траты, инкассация, наличные
на конец, комментарий «на что». В отличие от read-only блока «Касса за месяц» на
странице графика (он показывает только сданные кассы), регистр показывает ещё и
ПРОБЕЛЫ — дни, за которые кассу не сдали. Их и надо находить: пробел автоматом
снимает премию «передача смены» (500 ₽), а трата, забытая барменом и всплывшая в
чате, должна попасть в таблицу бухгалтерии.
...
- `routes/salary.py` — `GET /api/salary/cash-register` (чтение — всем),
  `PUT /api/salary/cash-register/shift/<id>` (правка — только администратору,
  без окна 72 ч).
- `templates/bonus.html` — раздел «Касса за месяц»: фильтр по точке, вид,
  поиск, правка строки, штраф, выгрузка CSV.

## Changelog

- 2026-08-07 — модуль создан (регистр на странице ЗП).
"""
