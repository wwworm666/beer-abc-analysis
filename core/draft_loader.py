...
вынесен сюда. Ключ кэша общий: при совпадении бара и периода страница /draft и
карточка дашборда читают одну запись, и цифры у них равны по построению.

В ключе есть версия формы ответа (v2 с 2026-09-21, когда в продажи добавилось
поле стиля DishGroup.ThirdParent). Без неё первые десять минут после выкатки
страница читала бы старую запись кэша, где стиля нет, и все кеги показывались бы
в «Без категории (Р)». Версию поднимать при каждом изменении НАБОРА ПОЛЕЙ в
запросах, иначе старый кэш переживёт выкатку.
...
# Версия формы сырья: см. докстроку модуля. Меняется вместе с набором полей.
CACHE_VERSION = 'v2'


def draft_kegs_cache_key(bar_name, date_from, olap_date_to):
    """Ключ кэша /draft: bar_name - iiko-имя бара или None (все бары)."""
    return f"draft_kegs_{CACHE_VERSION}_{bar_name or 'ALL'}_{date_from}_{olap_date_to}"
