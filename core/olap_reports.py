...
class OlapReports:
    ...
    def get_employee_discount_checks(self, date_from, date_to, bar_name=None, employee=None):
        ...
        OrderNum + OrderDiscount.Type + Delivery.CustomerName + Delivery.Phone.
        Ключ чека = дата + касса + номер, потому что OrderNum уникален только в пределах
        одного учётного дня и кассы (см. lessons.md).

        Гость: Delivery.CustomerName / Delivery.CustomerCardNumber заполнены ТОЛЬКО если на чеке
        проведена карта лояльности (опознан гость). Пустое имя/карта = карта не проводилась —
        это и есть пометка чеков с лояльностью (тот же механизм, что на странице /discounts).
        ...
        request = {
            ...
            "groupByRowFields": [
                ...
                "Delivery.CustomerName",       # ФИО гостя (заполнено, если проведена карта лояльности)
                "Delivery.CustomerCardNumber"  # карта/телефон гостя — идентификатор лояльности
                                               # (для гостей, зарегистрированных по телефону, тут телефон;
                                               #  Delivery.Phone в OLAP для зала всегда пуст — проверено 2026-06-17)
            ],
            "groupByColFields": [],
            "aggregateFields": [
