...
Ответить можно ТОЛЬКО в день смены. Приёмка описывает момент — «как я принял бар
утром»; ответ, данный через три дня, это уже не наблюдение, а воспоминание, и
возможность переписать его задним числом обнулила бы смысл доказательства.
Забыли — день остаётся `not_marked`, и это честнее, чем проставленное задним
числом «Чисто».

«День смены» здесь — РАБОЧИЕ сутки бара, а не календарные: бар закрывается в
03:00-04:00, и по календарю окно захлопывалось бы в полночь прямо посреди смены
(`core/msk_time.business_today`, рубеж 06:00 МСК). Бармен, открывший бар в
10:00, должен мочь поправить свою же приёмку в 00:30 — он ещё на работе.
...
def can_answer(shift_date, today) -> Tuple[bool, Optional[str]]:
    ...
    Окно — ровно день смены (см. докстроку модуля). Обе даты в ISO; `today`
    вызывающий берёт из `msk_time.business_today()`, то есть это РАБОЧИЕ сутки:
    после полуночи смена всё ещё «сегодняшняя».
    """
