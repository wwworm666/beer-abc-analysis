...
MSG = {
    # Отказ, который чаще всего означает НЕ ошибку: аккаунты владельца,
    # управляющего, бухгалтера к сотруднику графика не привязаны и привязаны
    # быть не должны. Поэтому сначала снимаем тревогу («вам сюда просто не
    # надо»), и только потом даём выход для бармена, которого забыли привязать:
    # если начать с «попросите администратора», человек не за смену читает это
    # как поломку и идёт её чинить.
    NOT_LINKED: (
        'Ваш аккаунт не привязан к сотруднику графика — скорее всего, вы не '
        'выходите в смены, и личный кабинет вам просто не нужен: здесь только '
        'своя смена, свои часы и своя зарплата. Всё остальное открыто как '
        'обычно — дашборд, график, отчёты в меню слева. Если вы всё-таки '
        'работаете в смену, попросите администратора связать логин с вашим '
        'именем: страница «Аккаунты» -> ваш логин -> столбец «Сотрудник».'
    ),
    UNKNOWN_EMPLOYEE: (
