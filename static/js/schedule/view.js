...
(function () {
    ...
    // Состояние модалки факта держит общий модуль fact_modal.js.
    var wishes = {};
    var wishTimers = {};
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // модалка факта: свои слушатели, закрытие по оверлею и тумблеры кассы
        // вешает сам общий модуль (fact_modal.js); нам остаётся сказать, что
        // делать после сохранения.
        S.factModal.init({ onSaved: reload });
        document.getElementById('shiftModal').addEventListener('click', function (e) {
            if (e.target === this) closeShiftModal();
        });
        // касса модалки смены (десктоп) — её тумблеры общий модуль не знает
    ...
    // ==================== Касса на смене ====================
    // Реализация вынесена в static/js/schedule/fact_modal.js — тот же код
    // использует личная страница /me. Здесь только локальные псевдонимы, чтобы
    // не переписывать вызовы в модалке смены (десктоп).
    var cashLocked = S.cashLocked;
    var wireCashToggles = S.wireCashToggles;
    var fillCashBlock = S.fillCashBlock;
    var readCashForSubmit = S.readCashForSubmit;
    var sendCash = S.sendCash;
    ...
    // ==================== Модалка факта ====================
    // Логика в static/js/schedule/fact_modal.js (общая с /me). Здесь только
    // обёртки: перерисовку после сохранения задаёт страница.
    function openFactModal(shift) { S.factModal.open(shift); }
    function closeFactModal() { S.factModal.close(); }
