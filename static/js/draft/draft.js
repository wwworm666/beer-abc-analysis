/* Страница «Розлив — ABC/XYZ и потери» (/draft; до 2026-09-19 — «Анализ проливов») —
   сборка экрана по макету владельца
...
(function () {
    ...
    function openBucket(key) {
        ...
        if (members.length) {
            ...
            html += '<div class="dr-empty">в этой группе сейчас нет кегов' +
    ...
    function xyzNote(keg) {
        ...
        // Огрызок периода короче недели остаётся в начале и в недели не входит;
        // без этой оговорки «на кране 0 из 4 нед.» рядом с литрами выглядит поломкой.
        if (keg.LitersOutsideWeeks > 0.005) {
            tail += ' ' + num(keg.LitersOutsideWeeks) + ' л пролито вне недельного окна ' +
                '(период не делится на целые недели, остаток в начале): в литрах выше ' +
                'они учтены, в XYZ не входят.';
        }
