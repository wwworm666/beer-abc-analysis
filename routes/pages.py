...
@pages_bp.route('/')
def index():
    """Главная — личная страница «Я» (`/me`).

    Редирект, а не второй `render_template`: у страницы остаётся ОДИН
    канонический адрес. Иначе одна и та же страница жила бы по двум адресам —
    подсветка активного пункта меню сравнивает href со `location.pathname` и на
    `/` погасла бы, закладки и история разъехались бы на две ветки, а любая
    правка ссылок требовала бы помнить про оба адреса.

    302, а не 301: 301 браузеры кэшируют бессрочно, и вернуть главную назад
    было бы нельзя без чистки кэша у каждого. Дашборд никуда не делся — он и
    раньше жил на `/dashboard`, ссылка в меню ведёт туда.
    """
    return redirect('/me')
...
def packaging():
    ...
    страница пересобрана в оформлении «Розлива» (docs/abc-xyz-analysis.md).
    """
...
def draft():
    ...
    и баланс склада на одном экране.
...
def waiters_redirect():
    """Анализ по барменам слит в «Розлив» (/draft, тогда «Анализ проливов»), вкладка «По барменам».
