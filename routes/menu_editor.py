...
Отличия от прежнего menu_tool:
  - объёмы разлива выбираются на карточке (vols: подмножество
    0,25/0,33/0,4/0,5/1,0, до 3 колонок; без vols — легаси-вид 0,25/0,4/0,5),
    переключателя A/B нет;
...
ITEM_FIELDS = [
    ...
    "tags", "ratings", "vols", "p025", "p033", "p04", "p05", "p10",
]
PRICE_FIELDS = ("p025", "p033", "p04", "p05", "p10")

# Реестр объёмов: (ключ, подпись на карточке, поле цены). Ключ = объём в литрах
# без запятой ("05" = 0,5 л, "10" = 1,0 л). Порядок кортежа = порядок колонок на
# карточке (по возрастанию объёма) — vols карточки всегда нормализуется к нему,
# порядок из UI/API значения не имеет. Зеркала: VOLUMES в templates/menu_editor.html
# (JS живого превью) и PORTION_FIELD в core/menu_pricing.py (цены из iiko).
VOLUMES = (
    ("025", "0,25", "p025"),
    ("033", "0,33", "p033"),
    ("04",  "0,4",  "p04"),
...
# Цена с таким числом знаков считается «широкой»: в трёх колонках «1,0 1900» на 56pt
# занимает 65 мм при ячейке 59 мм. Зеркало в JS редактора (WIDE_PRICE_DIGITS).
WIDE_PRICE_DIGITS = 4
...
def _decorate(item):
    ...
    # Цена от 4 знаков (обычно 1,0 л) в трёх колонках не влезает на штатном кегле:
    # класс .wide на секции цен ужимает все числа строки (menu_card.css).
    d["prices_wide"] = any(len(s["price"]) >= WIDE_PRICE_DIGITS for s in d["servings"])
    return d
