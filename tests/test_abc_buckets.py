...
def d(abc='A', markup=1.2, sales=10, weeks=4, weekly=None, outside=0.0, b_min=MARKUP_B_MIN):
    if weekly is None:
        weekly = [sales / weeks] * weeks if weeks else []
    return decide_bucket(abc, markup, sales, weeks, weekly, outside, b_min)
...
def test_no_cost_or_below_floor_goes_to_check():
    ...
    assert d(markup=0.99, b_min=KEG_MARKUP_B_MIN) == 'check'
    assert d(markup=1.0, b_min=KEG_MARKUP_B_MIN) == 'low_markup'
...
def test_low_markup_versus_core():
    assert d(abc='A', markup=MARKUP_B_MIN - 0.01) == 'low_markup'
    assert d(abc='B', markup=MARKUP_B_MIN) == 'core'
    assert d(abc='B', markup=0.99) == 'low_markup'
    assert d(abc='A', markup=1.9, b_min=KEG_MARKUP_B_MIN) == 'low_markup'
    assert d(abc='A', markup=2.0, b_min=KEG_MARKUP_B_MIN) == 'core'
...
def test_rule_texts_carry_the_thresholds():
    ...
    assert n in bucket_rule_text('core', MARKUP_B_MIN, 'pieces', 30)
    assert '100%' in bucket_rule_text('core', MARKUP_B_MIN, 'pieces', 30)
    assert '50%' in bucket_rule_text('check', MARKUP_B_MIN, 'pieces', 30)
    kegs = bucket_rule_text('low_markup', KEG_MARKUP_B_MIN, 'portions', 7)
    assert '200%' in kegs and 'порций' in kegs and n in kegs
    assert '100%' in bucket_rule_text('check', KEG_MARKUP_B_MIN, 'portions', 7)
    assert str(MIN_DAYS_FOR_NEGATIVE_VERDICT) in bucket_rule_text('weak', MARKUP_B_MIN, 'pieces', 30)
