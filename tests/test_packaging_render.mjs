...
test('решения по ассортименту: имена и правила живут в ядре, страница печатает', () => {
    const buckets = read('core/abc_buckets.py');
    for (const key of ['core', 'low_markup', 'weak', 'few', 'new', 'check']) {
        assert.ok(buckets.includes(`'${key}'`), `в ядре нет группы ${key}`);
    }
    for (const name of ['Основа выручки', 'Низкая наценка', 'Слабые продажи', 'Мало продаж',
                        'Новинки периода', 'Сверить учёт']) {
        assert.ok(buckets.includes(name), `в ядре нет группы «${name}»`);
    }
    // На фронте нет своего словаря групп: карточки приходят с сервера (data.buckets)
...
test('прочерк вместо буквы объяснён, а не выдан за оценку', () => {
    // В таблице буквы нет только внутри кода («?»), колонки XYZ с прочерком больше
    // нет (2026-09-26); в карточке позиции прочерк остался — там он и объяснён.
    assert.match(html, /«\?» вместо буквы означает, что данных не хватило/,
        'в легенде не сказано, что «?» — это не «плохо»');
