...
# поэтому каждая строка обязана быть временной и объяснённой.
ALLOWED = {
    'chz_test/debug/token.json':
        'Кэш токена ЧЗ, просрочен 12.04.2026, закоммичен до появления этой '
        'проверки. Снять с учёта: git rm --cached chz_test/debug/token.json '
        '(правило в .gitignore уже есть). После этого строку убрать отсюда, '
        'иначе исключение начнёт прикрывать новый секрет.',
}
